...
var Protein = function() { 
  ...
  this.load = function(protein_data) {
    ...
    this.make_atoms_from_pdb_lines(protein_data['pdb_atom_lines']);
    this.make_residues();
...
var Controller = function(scene) {
  ...
  this.clear_selected = function() {
    this.protein.clear_selected();
    this.scene.current_view.selected = 
        this.make_selected_str();
    this.scene.changed = true;
  ...
  this.view_from_dict = function(flat_dict) {
    ...
    view.order = grab_float(flat_dict.order);
    view.res_id = flat_dict.res_id;
    view.i_atom = grab_float(flat_dict.i_atom);
    ...
    if ('show_all_atom' in flat_dict) {
      view.show.all_atom = grab_boolean(flat_dict.show_all_atom);
    } else if ('show_backbone' in flat_dict) {
      view.show.all_atom = grab_boolean(flat_dict.backbone);
    }
    view.show.all_atom = grab_boolean(flat_dict.show_all_atom);
    view.show.sidechain = grab_boolean(flat_dict.show_sidechain);
    view.show.hydrogen = grab_boolean(flat_dict.show_hydrogen);
    view.show.trace = grab_boolean(flat_dict.show_trace);
    view.show.water = grab_boolean(flat_dict.show_water);
    view.show.ribbon = grab_boolean(flat_dict.show_ribbon);
    view.show.ligands = grab_boolean(flat_dict.show_ligands);

    view.abs_camera.pos.x = grab_float(flat_dict.camera_pos_x);
    view.abs_camera.pos.y = grab_float(flat_dict.camera_pos_y); 
    view.abs_camera.pos.z = grab_float(flat_dict.camera_pos_z);
    view.abs_camera.up_v.x = grab_float(flat_dict.camera_up_x); 
    view.abs_camera.up_v.y = grab_float(flat_dict.camera_up_y); 
    view.abs_camera.up_v.z = grab_float(flat_dict.camera_up_z);
    view.abs_camera.in_v.x = grab_float(flat_dict.camera_in_x); 
    view.abs_camera.in_v.y = grab_float(flat_dict.camera_in_y); 
    view.abs_camera.in_v.z = grab_float(flat_dict.camera_in_z); 

    view.camera.z_front = grab_float(flat_dict.z_front);
    view.camera.z_back = grab_float(flat_dict.z_back);
    view.camera.zoom = grab_float(flat_dict.zoom);
    view.abs_camera.z_front = grab_float(flat_dict.z_front);
    view.abs_camera.z_back = grab_float(flat_dict.z_back);
    view.abs_camera.zoom = grab_float(flat_dict.zoom);
  ...
  this.sort_views_by_order = function() {
    var order_sort = function(a, b) {
      return grab_float(a.order) - grab_float(b.order);
    }
